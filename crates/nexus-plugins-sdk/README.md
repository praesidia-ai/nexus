...
Plugin SDK for Nexus — manifest, hooks, capabilities for external developers
...
Run `cargo doc -p nexus-plugins-sdk --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-plugins-sdk` and `cargo clippy -p nexus-plugins-sdk -- -D warnings`.
...
```bash
cargo build -p nexus-plugins-sdk
cargo test  -p nexus-plugins-sdk
```
