...
Single embed surface for Nexus: ZeroClaw agent pool, workflow engine, Praesidia policy, project store
...
Run `cargo doc -p nexus-core --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-core` and `cargo clippy -p nexus-core -- -D warnings`.
