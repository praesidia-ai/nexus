...
LLM provider abstraction for Nexus — traits for multi-provider completion, streaming, embeddings
...
Run `cargo doc -p nexus-providers --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-providers` and `cargo clippy -p nexus-providers -- -D warnings`.
...
```bash
cargo build -p nexus-providers
cargo test  -p nexus-providers
```
