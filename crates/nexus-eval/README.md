...
Evaluation and benchmarking framework for Nexus — suites, runners, LLM-as-judge, regression detection
...
Run `cargo doc -p nexus-eval --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-eval` and `cargo clippy -p nexus-eval -- -D warnings`.
...
```bash
cargo build -p nexus-eval
cargo test  -p nexus-eval
```
