...
Embedded knowledge graph — entity extraction, temporal edges, contradiction detection, provenance tracking
...
Run `cargo doc -p nexus-graph --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-graph` and `cargo clippy -p nexus-graph -- -D warnings`.
...
```bash
cargo build -p nexus-graph
cargo test  -p nexus-graph
```
