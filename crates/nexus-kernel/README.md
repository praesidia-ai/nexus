...
Agent OS kernel — scheduler, event bus, reactive agents
...
Run `cargo doc -p nexus-kernel --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-kernel` and `cargo clippy -p nexus-kernel -- -D warnings`.
