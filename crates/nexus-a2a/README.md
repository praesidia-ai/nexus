...
Agent-to-Agent (A2A) protocol implementation for Nexus
...
Run `cargo doc -p nexus-a2a --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-a2a` and `cargo clippy -p nexus-a2a -- -D warnings`.
...
```bash
cargo build -p nexus-a2a
cargo test  -p nexus-a2a
```
