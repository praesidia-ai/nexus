...
Parallel agent execution runtime — process isolation, job queue, durable checkpoints, resource budgeting
...
Run `cargo doc -p nexus-runtime --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-runtime` and `cargo clippy -p nexus-runtime -- -D warnings`.
