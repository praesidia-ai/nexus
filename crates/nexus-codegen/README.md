...
Multi-agent code generation engine — specialized agent teams for full-stack app generation
...
Run `cargo doc -p nexus-codegen --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-codegen` and `cargo clippy -p nexus-codegen -- -D warnings`.
...
```bash
cargo build -p nexus-codegen
cargo test  -p nexus-codegen
```
