# `nexus-integrations-ext`

Deep integrations — GitHub PRs, Slack/Discord bots, Jira/Linear, Stripe, deployment platforms
...
Run `cargo doc -p nexus-integrations-ext --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-integrations-ext` and `cargo clippy -p nexus-integrations-ext -- -D warnings`.
...
```bash
cargo build -p nexus-integrations-ext
cargo test  -p nexus-integrations-ext
```
