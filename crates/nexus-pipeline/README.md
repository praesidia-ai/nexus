...
Deterministic agent execution pipeline — blueprints, feedback gates, and interleaved execution
...
Run `cargo doc -p nexus-pipeline --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-pipeline` and `cargo clippy -p nexus-pipeline -- -D warnings`.
...
```bash
cargo build -p nexus-pipeline
cargo test  -p nexus-pipeline
```
