...
Nexus workflow demo — runs a full app-build pipeline with the ZeroClaw agent team
...
Run `cargo doc -p agent-demo --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p agent-demo` and `cargo clippy -p agent-demo -- -D warnings`.
...
```bash
cargo build -p agent-demo
cargo test  -p agent-demo
```
