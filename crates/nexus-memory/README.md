...
Persistent agent memory system — episodic, semantic, and vector storage
...
Run `cargo doc -p nexus-memory --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-memory` and `cargo clippy -p nexus-memory -- -D warnings`.
...
```bash
cargo build -p nexus-memory
cargo test  -p nexus-memory
```
