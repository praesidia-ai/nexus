...
First-party integrations for GitHub, databases, and notifications
...
Run `cargo doc -p nexus-integrations --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-integrations` and `cargo clippy -p nexus-integrations -- -D warnings`.
...
```bash
cargo build -p nexus-integrations
cargo test  -p nexus-integrations
```
