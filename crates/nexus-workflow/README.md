...
Persistent, resumable, branching workflow execution engine with SQLite-backed state
...
Run `cargo doc -p nexus-workflow --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-workflow` and `cargo clippy -p nexus-workflow -- -D warnings`.
...
```bash
cargo build -p nexus-workflow
cargo test  -p nexus-workflow
```
