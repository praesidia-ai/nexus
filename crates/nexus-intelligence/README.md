...
Intelligence traits and types for Nexus — intent analysis, decisions, product intelligence, learning
...
Run `cargo doc -p nexus-intelligence --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-intelligence` and `cargo clippy -p nexus-intelligence -- -D warnings`.
...
```bash
cargo build -p nexus-intelligence
