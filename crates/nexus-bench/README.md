...
Criterion benchmark suite for Nexus hot paths
...
Run `cargo doc -p nexus-bench --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-bench` and `cargo clippy -p nexus-bench -- -D warnings`.
