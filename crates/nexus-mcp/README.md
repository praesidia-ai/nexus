...
MCP (Model Context Protocol) client and server for Nexus
...
Run `cargo doc -p nexus-mcp --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-mcp` and `cargo clippy -p nexus-mcp -- -D warnings`.
