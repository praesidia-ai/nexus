...
SQLite persistence and session domain API for Nexus
...
Run `cargo doc -p nexus-store --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-store` and `cargo clippy -p nexus-store -- -D warnings`.
