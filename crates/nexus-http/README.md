...
Axum HTTP server exposing all Nexus capabilities: projects, SSE chat, tables, agents, workflows, portals
...
Run `cargo doc -p nexus-http --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-http` and `cargo clippy -p nexus-http -- -D warnings`.
...
```bash
cargo build -p nexus-http
cargo test  -p nexus-http
```
