...
Nexus Agent Package Manager — install, publish, and version agent packages
...
Run `cargo doc -p nexus-pkg --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-pkg` and `cargo clippy -p nexus-pkg -- -D warnings`.
...
```bash
cargo build -p nexus-pkg
cargo test  -p nexus-pkg
```
