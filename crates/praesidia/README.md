...
Praesidia policy engine and cloud client for Nexus — publish projects, register agents, enforce YAML policies
...
Run `cargo doc -p praesidia --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p praesidia` and `cargo clippy -p praesidia -- -D warnings`.
...
```bash
cargo build -p praesidia
cargo test  -p praesidia
```
