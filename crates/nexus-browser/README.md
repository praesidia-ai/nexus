...
Browser automation agent — Playwright/CDP-based web interaction, E2E testing, visual verification
...
Run `cargo doc -p nexus-browser --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-browser` and `cargo clippy -p nexus-browser -- -D warnings`.
...
```bash
cargo build -p nexus-browser
cargo test  -p nexus-browser
```
