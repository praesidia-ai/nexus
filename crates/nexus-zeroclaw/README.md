...
Nexus wrapper around the ZeroClaw agent runtime — named agents, roles, roster, inter-agent messaging
...
Run `cargo doc -p nexus-zeroclaw --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-zeroclaw` and `cargo clippy -p nexus-zeroclaw -- -D warnings`.
...
```bash
cargo build -p nexus-zeroclaw
cargo test  -p nexus-zeroclaw
```
