...
Quality scoring traits and types for Nexus — taste scoring, guarantees, fix plans
...
Run `cargo doc -p nexus-quality-core --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-quality-core` and `cargo clippy -p nexus-quality-core -- -D warnings`.
...
```bash
cargo build -p nexus-quality-core
cargo test  -p nexus-quality-core
```
