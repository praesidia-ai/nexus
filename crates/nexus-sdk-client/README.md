...
Rust HTTP client SDK for embedding Nexus agents in applications
...
Run `cargo doc -p nexus-sdk-client --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-sdk-client` and `cargo clippy -p nexus-sdk-client -- -D warnings`.
...
```bash
cargo build -p nexus-sdk-client
cargo test  -p nexus-sdk-client
```
