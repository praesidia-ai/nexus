...
Agent runtime types, traits, and team definitions for Nexus
...
Run `cargo doc -p nexus-agents-core --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-agents-core` and `cargo clippy -p nexus-agents-core -- -D warnings`.
...
```bash
cargo build -p nexus-agents-core
cargo test  -p nexus-agents-core
```
