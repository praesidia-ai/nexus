...
Nexus CLI — AI-native application generation and agent teams
...
Run `cargo doc -p cli --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p cli` and `cargo clippy -p cli -- -D warnings`.
