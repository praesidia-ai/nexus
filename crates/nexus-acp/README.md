...
Agent Client Protocol (ACP) adapter — expose Nexus to Zed, JetBrains, and the wider ACP registry via JSON-RPC over stdio
...
Run `cargo doc -p nexus-acp --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-acp` and `cargo clippy -p nexus-acp -- -D warnings`.
...
```bash
cargo build -p nexus-acp
cargo test  -p nexus-acp
```
