...
Three-layer context engine with adaptive compression for agent memory management
...
Run `cargo doc -p nexus-context --open` for the rendered API reference.
...
3. Add tests next to the change (unit tests in-file, integration tests
   under `tests/`).
4. Run `cargo test -p nexus-context` and `cargo clippy -p nexus-context -- -D warnings`.
...
```bash
cargo build -p nexus-context
cargo test  -p nexus-context
```
